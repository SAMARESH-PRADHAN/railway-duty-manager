...
//* Applies sql/schema.sql to the database at LOCAL_DATABASE_URL or NEON_DATABASE_URL,
//* depending on DB_DRIVER ("local" or "neon"). Uses the Pool client (not the
//* tagged-template `sql` helper) because schema.sql contains multiple
//* semicolon-separated statements, which the lightweight `neon()` tagged-template
//* client does not support in one call.
...
async function main() {
  ...
  console.log(`Applying schema.sql using ${driver} driver...`);
