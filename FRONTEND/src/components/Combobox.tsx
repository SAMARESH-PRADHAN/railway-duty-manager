...
export function Combobox({ options, value, onChange, placeholder = "Select…", emptyText = "No results.", className, disabled, allowCreate, onCreate }: Props) {
  const [open, setOpen] = useState(false);
      ...
      <PopoverContent className="p-0 w-[--radix-popover-trigger-width] max-w-[95vw]" align="start">
        <Command>
          ...
          <CommandList className="max-h-[50vh]">
            ...
            <CommandGroup>
              {options.map((o) => (
                <CommandItem key={o.value} value={`${o.label} ${o.hint ?? ""}`}
                  onSelect={() => { onChange(o.value); setQuery(""); setOpen(false); }}>
                  <Check className={cn("mr-2 h-4 w-4", value === o.value ? "opacity-100" : "opacity-0")} />
                  <div className="flex-1 min-w-0">
                    <div className="truncate">{o.label}</div>
                    {o.hint && <div className="text-[10px] text-slate-500 truncate">{o.hint}</div>}
