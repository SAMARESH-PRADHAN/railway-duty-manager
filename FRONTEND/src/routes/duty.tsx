...
export default function DutyPage() {
  ...
  const [unsavedModal, setUnsavedModal] = useState<{ open: boolean; proceed: (() => void) | null }>(
    { open: false, proceed: null },
  );
  ...
  const employeeBatch = batches.find(
    (b) => b.name.trim().toLowerCase() === emp?.presentBatch.trim().toLowerCase(),
  );
  ...
  const endDate = useMemo(
    () => (startDate ? format(addDays(parseISO(startDate), 13), "yyyy-MM-dd") : ""),
    [startDate],
  );

  const overlaps = useMemo(() => {
    if (!employeeId || !startDate) return false;
    return dutySheets.some(
      (s) =>
        s.id !== sheetId &&
        !s.isDraft &&
    ...
    () =>
      days.filter(
        (d) =>
          d.isRestDay &&
          !d.actualIsRest &&
          d.actualHours - (d.leave && d.leave !== "None" && d.leave !== "CR" ? 7 : 0) > 0,
      ),
    [days],
  );
  ...
  const updateDay = (idx: number, patch: Partial<DutyDay>) => {
    setDays((prev) => {
      ...
      merged.rosteredHours =
        patch.rosteredHours !== undefined ? patch.rosteredHours : sumSlots(merged.rosteredSlots);
      merged.actualHours =
        patch.actualHours !== undefined ? patch.actualHours : sumSlots(merged.actualSlots);
    ...
    if (leave === "CR") {
      const used = new Set(
        days
          .map((dd, k) =>
            k !== idx && dd.leave === "CR"
              ? (dd.description.match(/CR of (\d{2}\.\d{2}\.\d{4})/)?.[1] ?? "")
              : "",
          )
          .filter(Boolean),
      );
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[#0b2545]">
              {existing ? "Edit" : "New"} Extra Hour Slip
            </h1>
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Step 1 — Select Employee</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {emp && (
              <div className="rounded-lg border bg-slate-50 p-4 grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                ...
                <div>
                  ...
                  <div className="font-semibold">{emp.tokenNo}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-500">PF No</div>
                  <div className="font-semibold">{emp.pfNumber}</div>
                </div>
                <div>
                ...
                <div>
                  <div className="text-xs text-slate-500">Group Type</div>
            ...
            <div className="flex justify-end">
              ...
              >
                Next: Select Trains <ArrowRight className="h-4 w-4 ml-1" />
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {step === 2 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Step 2 — Select Train(s)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {activeTr.map((t) => {
                ...
                return (
                  <label
                    key={t.id}
                    className={`flex items-center gap-3 rounded-md border p-3 cursor-pointer ${on ? "border-blue-500 bg-blue-50" : "hover:bg-slate-50"}`}
                  >
            ...
            <div className="flex justify-between flex-wrap gap-2">
              <Button variant="outline" onClick={() => setStep(1)}>
                <ArrowLeft className="h-4 w-4 mr-1" /> Back: Select Employee
              </Button>
              <Button
                disabled={trainIds.length === 0 && !manualTrainNote.trim()}
                onClick={() => setStep(3)}
                className="bg-[#0b2545] hover:bg-[#0b2545]/90"
              >
                Next: Select Date <ArrowRight className="h-4 w-4 ml-1" />
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {step === 3 && (
        <>
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Step 3 — Select Start Date</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
          ...
          <div className="flex justify-between flex-wrap gap-2">
            <Button variant="outline" onClick={() => setStep(2)}>
              <ArrowLeft className="h-4 w-4 mr-1" /> Back: Select Trains
            </Button>
            <Button
              disabled={!startDate}
              onClick={() => setStep(4)}
          ...
          <CardContent className="space-y-4">
                    ...
                    return (
                      <tr key={d.date} className={bothRest ? "bg-amber-50" : ""}>
                        ...
                        <td className={`p-2 border align-top ${rosteredCellBg}`}>
                          <SlotEditor
                            slots={d.rosteredSlots}
                            onChange={(s) => setRosteredSlots(i, s)}
                            isRest={d.isRestDay}
                            onToggleRest={toggleRosteredRest}
                          />
                        </td>
                        <td className={`p-2 border align-top ${rosteredCellBg}`}>
                          <Input
                            ...
                            onChange={(e) =>
                              updateDay(i, { rosteredHours: Number(e.target.value) })
                            }
                          />
                        </td>
                        <td className={`p-2 border align-top ${actualCellBg}`}>
                          <SlotEditor
                            ...
                            onChange={(s) =>
                              updateDay(i, {
                                actualSlots: s,
                                actualHours: sumSlots(s) + leaveHoursCredit(d.leave),
                              })
                            }
                        ...
                        <td className="p-2 border align-top">
                          <div className="flex items-start gap-1">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                updateDay(i, { description: has2 ? line1 : `${line1}\n` })
                              }
            ...
            <div className="flex justify-between flex-wrap gap-2">
              <Button variant="outline" onClick={() => setStep(3)}>
                <ArrowLeft className="h-4 w-4 mr-1" /> Back: Select Date
              </Button>
      ...
      <Dialog open={sundayModalOpen} onOpenChange={setSundayModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              The selected date ({pendingStart && fmtDate(pendingStart)}) is not a Sunday. Duty
              periods conventionally start on Sunday. Do you want to continue with this date anyway?
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        ...
        onOpenChange={(o) => {
          if (!o) setUnsavedModal({ open: false, proceed: null });
        }}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
          ...
          <DialogFooter className="flex-col-reverse sm:flex-row gap-2 sm:justify-between">
            <Button
              variant="outline"
              onClick={() => setUnsavedModal({ open: false, proceed: null })}
            >
              Cancel
            </Button>
            <div className="flex gap-2">
    ...
    <div className="flex items-center gap-1 text-xs">
      {labels.map((l, i) => (
        <div key={l} className="flex items-center gap-1">
          <div
            className={`h-6 w-6 rounded-full grid place-items-center font-bold text-[10px] ${i + 1 <= step ? "bg-[#0b2545] text-white" : "bg-slate-200 text-slate-500"}`}
          >
            {i + 1}
          </div>
          <span
            className={`hidden sm:inline ${i + 1 === step ? "font-semibold" : "text-slate-500"}`}
          >
            {l}
          </span>
...
function StatBox({
  ...
  return (
    <div
      className={`rounded-lg border p-3 ${highlight ? "bg-emerald-50 border-emerald-300" : "bg-slate-50"}`}
    >
    ...
    <div className="space-y-1">
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1">
        {slots.length < 4 && (
          <button
            type="button"
            onClick={() => onChange([...slots, { from: "08:00", to: "16:00" }])}
...
function CopyFromPastDuty({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Copy roster from a past duty sheet (optional)</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="text-xs text-slate-500">
          Copies the entire 14-day roster (rostered timings) from a past duty sheet into this new
          period.
        </div>
          ...
          <div>
            ...
            {noHistory ? (
              ...
              <Combobox
                ...
                options={past.map((s) => ({
                  value: s.id,
                  label: `${fmtDate(s.periodStartDate)} → ${fmtDate(s.periodEndDate)}`,
                  hint: `${s.days.length} days`,
                }))}
        ...
        <div className="flex justify-end">
          <Button
            disabled={noHistory || !activeSheet || !startDate}
            onClick={applyCopy}
            className="bg-[#0b2545] hover:bg-[#0b2545]/90"
          >
            Copy and Next <ArrowRight className="h-4 w-4 ml-1" />
          </Button>
        </div>
      </CardContent>
    </Card>
